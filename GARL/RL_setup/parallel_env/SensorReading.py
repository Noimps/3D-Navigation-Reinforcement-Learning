...
class SensorReading():
    ...
    def start_lidars(self):
        partition_name = f"sim{self.instance_id}"
        ...
        port_name = self.world_name.split("_")[-1]

        self.lidar_subscriptions.append(self.down_node.subscribe(topic=f"/{port_name}/down", msg_type=laserscan_pb2.LaserScan, callback=self.make_lidar_callback("down", 16)))
        self.lidar_subscriptions.append(self.up_node.subscribe(topic=f"/{port_name}/up"  , msg_type=laserscan_pb2.LaserScan, callback=self.make_lidar_callback("up", 20)))
        self.lidar_subscriptions.append(self.front_node.subscribe(topic=f"/{port_name}/front", msg_type=laserscan_pb2.LaserScan, callback=self.make_lidar_callback("front", 0)))
        self.lidar_subscriptions.append(self.back_node.subscribe (topic=f"/{port_name}/back", msg_type=laserscan_pb2.LaserScan, callback=self.make_lidar_callback("back", 4)))
        self.lidar_subscriptions.append(self.left_node.subscribe (topic=f"/{port_name}/left", msg_type=laserscan_pb2.LaserScan, callback=self.make_lidar_callback("left",8)))
        self.lidar_subscriptions.append(self.right_node.subscribe(topic=f"/{port_name}/right", msg_type=laserscan_pb2.LaserScan, callback=self.make_lidar_callback("right",12)))
